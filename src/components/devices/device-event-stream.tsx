...
import { fetchDeviceEvents } from "@/lib/actions/devices";
import type { EventRow, PaginatedEvents, Profile } from "@/lib/queries/types";
import { ChevronDown, ChevronRight, RefreshCw } from "lucide-react";

const PAGE_SIZE = 50;

interface DeviceEventStreamProps {
  deviceId: string;
  range: string;
...
export function DeviceEventStream({
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const result = await fetchDeviceEvents(deviceId, range, PAGE_SIZE, 0);
  ...
  const loadMore = useCallback(async () => {
    setLoadingMore(true);
    try {
      const result = await fetchDeviceEvents(deviceId, range, PAGE_SIZE, offset);
        ...
        {events.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground/70 mt-1">
              This device has no events in the selected time range.
            </p>
          </div>
        ) : (
          <>
...
function EventRow({ event, isExpanded, onToggle, profiles }: EventRowProps) {
  return (
    <>
